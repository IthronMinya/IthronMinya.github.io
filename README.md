...
However, as we all know, not all machine learning applications are critical, take a look at Amazon product recommendations or youtube video suggestions. User tailored recommendations might highly increase the platform use or lead to higher revenues but are not lifethreatening if the predictions are wrong sometimes. So do we need to look into these as well, or is it just a waste of time?

Well, why stop after finding out more about which input features lead to certain predictions? Using explainability methods we might find particular classes of failure cases and possible input biases, or other problems of the prediction with which one can furhter improve on model performance.
...
[<img src="https://render.githubusercontent.com/render/math?math=E_{GRAD}(I_0) = \frac{\delta S_a}{\delta I}|_{I = I_a}">](https://latex.codecogs.com/svg.image?E_{GRAD}(I_0)&space;=&space;\frac{\delta&space;S_a}{\delta&space;I}|_{I&space;=&space;I_a})
